  ...
  <div>
    ...
    <el-card class="box-card">
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-select v-model="selectedCinemaBrandId" placeholder="请选择影院品牌" clearable>
            <el-option v-for="item in cinemaBrandList" :key="item.cinemaBrandId" :label="item.cinemaBrandName"
      ...
      <el-table :data="cinemaList" style="width: 100%" border stripe >
        ...
        <el-table-column prop="sysCinemaArea.province" label="省份"></el-table-column>
          ...
          <template slot-scope="scope">
            <el-tooltip effect="dark" content="修改影院信息" placement="top" :enterable="false" :open-delay="500">
              <el-button type="primary" icon="el-icon-edit" size="mini"
                @click="showEditDialog(scope.row.cinemaId)"></el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除影院" placement="top" :enterable="false" :open-delay="500">
              <el-button type="danger" icon="el-icon-delete" size="mini"
                @click="deleteCinemaById(scope.row.cinemaId)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--添加影院对话框-->
    <el-dialog title="添加影院" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed"  @opened="initMap">
      <!--内容主题区-->
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="150px">
        ...
        <el-form-item label="影院区域" prop="cinemaAreaId">
          <el-select v-model="addForm.cinemaAreaId" placeholder="请选择影院区域" clearable>
            <el-option v-for="item in cinemaAreaList" :key="item.cinemaAreaId"
              :label="`${(item.province)}${(item.city)}${(item.cinemaAreaName)}`" :value="item.cinemaAreaId">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="入驻日期" prop="entryDate">
          <el-date-picker v-model="addForm.entryDate" value-format="yyyy-MM-dd" type="date" placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="影院图片">
          <el-upload action="" list-type="picture-card" :auto-upload="false" :file-list="pics" :on-change="handleChange"
            :on-success="handleSuccess" :on-error="handleError" ref="pictureRef" :http-request="submitFile">
    ...
    <el-dialog title="修改影院" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="150px">
        ...
        <el-form-item label="入驻日期" prop="entryDate">
          <el-date-picker v-model="editForm.entryDate" value-format="yyyy-MM-dd" type="date" placeholder="选择日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="影院图片">
          <el-upload action="" list-type="picture-card" :auto-upload="false" :file-list="pics" :on-change="handleChange"
            :on-success="handleSuccess" :on-error="handleError" ref="pictureEditRef" :http-request="submitFile">
            <i slot="default" class="el-icon-plus"></i>
            <div slot="file" slot-scope="{file}">
              <img class="el-upload-list__item-thumbnail" :src="file.url" alt="">
              <span class="el-upload-list__item-actions">
...
<script>
import * as loadBMap from '@/utils/loadBMap.js'
...
export default {
  ...
  methods: {
    ...
    initMap() {
      ...
      // 1、挂载地图
      map = new BMap.Map('map-container', { enableMapClick: false })
    ...
    getAddrByPoint(point) {
      ...
      geco.getLocation(point, function (res) {
       ...
       // mk.panTo(point)
        xx.form.address = res.address
        xx.form.addrPoint = point
      })
    },
    // 8-1、地址搜索
    ...
    /* mapPositoin() {
      const _this = this;
      const address = this.getLabls() + this.address
      //创建地址解析器实例
      ...
      myGeo.getPoint(address, function (point) {
        if (point) {
          ...
          _this.map.addOverlay(new BMapGL.Marker(point, { title: address }))
        } else {
          alert('您选择的地址没有解析到结果！');
        }
      }, '北京市')
